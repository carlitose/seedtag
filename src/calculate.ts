...
const closestEnemies = (scanArray: Array<Radar>): Array<Radar> =>
  scanArray.slice().sort((a, b) => a.distance - b.distance);
const furtherEnemies = (scanArray: Array<Radar>): Array<Radar> =>
  scanArray.slice().sort((a, b) => b.distance - a.distance);
const assistAllies = (scanArray: Array<Radar>): Array<Radar> =>
  scanArray.filter((element) => element.thereAreAllies());
const avoidCrossFire = (scanArray: Array<Radar>): Array<Radar> =>
  scanArray.filter((element) => !element.thereAreAllies());
const prioritizeMech = (scanArray: Array<Radar>): Array<Radar> =>
...
const avoidMech = (scanArray: Array<Radar>): Array<Radar> =>
  scanArray.filter((element) => !element.isAMech());
const removeTooDistanceElements = (scanArray: Array<Radar>): Array<Radar> =>
